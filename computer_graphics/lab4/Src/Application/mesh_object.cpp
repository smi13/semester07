...
MeshObject::MeshObject() 
{
   ...
   _textures = 0;
   _numMaterials = 0;
   Object::_indicesCount = 0;
   Object::_verticesCount = 0;
   Object::_vbuf = 0;
   Object::_ibuf = 0;
}

MeshObject::MeshObject( const char *name ) : Object(std::string(name))
{
   _mesh = 0;
   _materials = 0;
   _textures = 0;
...
void ShapeCreator::createTeapot( MeshObject *obj, Material mat )
{
   HRESULT hr = D3DXCreateTeapot(getDevice(), &obj->_mesh, 0);

   if (SUCCEEDED(hr))
   {
      obj->_numMaterials = 1;
      obj->_materials = new D3DMATERIAL9[1];
      obj->_materials[0] = mat;
   }
}

void ShapeCreator::createBox( MeshObject *obj, Material mat, float width, float height, float depth )
{
   HRESULT hr = D3DXCreateBox(getDevice(), width, height, depth, &obj->_mesh, 0);
...
void ShapeCreator::createCylinder( MeshObject *obj, Material mat, float radius, float length, int slices, int stacks )
{
   HRESULT hr = D3DXCreateCylinder(getDevice(), radius, radius, length, slices, stacks, &obj->_mesh, 0);
